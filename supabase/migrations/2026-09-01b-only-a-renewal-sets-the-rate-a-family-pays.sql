/* ONLY A RENEWAL SETS THE RATE A FAMILY PAYS
   ==================================================================
   scope: shared        affects: reminder_queue(), mezzo only in practice

   2026-09-01a taught the queue to quote the monthly rate a family last
   actually paid. It took the last payment of ANY kind, which is wrong
   the moment a tenant records anything that is not a monthly fee.

   The platform already holds the counter-examples:

       genalpha  admission   40 payments, up to 29,000
       genalpha  custom       4 payments, 600-750
       raj       admission    7 payments, up to  9,000

   An admission fee is a one-off. Divided by its months it is not a
   monthly rate, and letting one become "what this family pays" would
   put 29,000 into a reminder. Mezzo has only renewals today — all
   sixty-three of them — so nothing changes for it now; this is the
   guard for the first time it takes an admission fee, which is exactly
   when nobody would be looking.

   A tenant whose payments carry no kind at all (demo, leo) now matches
   nothing here and falls back to resolve_fee(), which is the behaviour
   they had before 2026-09-01a. Falling back is the safe direction: it
   quotes the list price rather than a wrong one.

   Requested explicitly: this stays mezzo-only, and renewal-only.
   ================================================================== */
...
declare v_bad int;
begin
  /* no non-renewal payment may set anybody's rate */
  select count(*) into v_bad
    from tenants t
    join lateral reminder_queue(t.id) q on true
   where q.fee_source = 'last_paid'
     and not exists (
       select 1 from payments p
        where p.tenant_id = t.id and p.enrollment_id = q.enrollment_id
          and p.kind = 'renewal' and p.status <> 'void'
          and coalesce(p.months,0) > 0);
  if v_bad <> 0 then
    raise exception '% rows are priced from something that is not a renewal', v_bad;
  end if;

  /* and it is still only mezzo */
  select count(*) into v_bad
    from tenants t join lateral reminder_queue(t.id) q on true
   where t.id <> 'mezzo' and q.fee_source = 'last_paid';
  if v_bad <> 0 then
    raise exception '% rows outside mezzo are priced from a payment', v_bad;
  end if;
end $$;
